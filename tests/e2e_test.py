...
dump_uint32 = io.dump_int(bits=32, signed=False)
...
def test_append_vs_create():
    ...
    # first write
    file_a = tmpfile()
    _, index_a = make_dataset(dataset, file_a, dumpers)

    # second write
    file_b = tmpfile()
    data = IndexedRecordDataset(file_b, dumpers=dumpers, create=True)
...
def test_quick_removal():
    ...
    # Create dataset
    make_dataset([[i] for i in data_orig], name, [io.dump_int])
    data = IndexedRecordDataset(name, loaders=[io.load_int])
...
def test_defrag():
    ...
    # Create dataset
    data_orig = [random.randint(0, n) for _ in range(n)]
    make_dataset([[i] for i in data_orig], name_1, [io.dump_float])
    data = IndexedRecordDataset(
        name_1, dumpers=[io.dump_float], loaders=[io.load_float]
    )
    ...
    # Truncate
    data.defrag(name_2)
    data_defrag = IndexedRecordDataset(name_2, data.loaders)
...
def test_partial_take():
    ...
    data = IndexedRecordDataset(
        name,
        create=True,
        dumpers=[io.dump_int] * 3,
        loaders=[io.load_int] * 3,
    )
